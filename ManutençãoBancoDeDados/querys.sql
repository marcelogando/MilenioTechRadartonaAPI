...
-- LocalizacaoRadares TRIGGER
CREATE OR REPLACE FUNCTION atualiza_LocalizacaoRadares() RETURNS trigger AS $atualiza_LocalizacaoRadares$
    DECLARE
    ...
    BEGIN
        DELETE FROM camadavisualizacao."LocalizacaoRadares";
        INSERT INTO camadavisualizacao."LocalizacaoRadares" ("Id", "JsonRetorno") VALUES (1, omgjson);
		
		RETURN NEW;
    END;
$atualiza_LocalizacaoRadares$ LANGUAGE plpgsql;

CREATE TRIGGER t_LocalizacaoRadares AFTER INSERT OR UPDATE
ON public."BaseRadares"
FOR EACH STATEMENT EXECUTE PROCEDURE atualiza_LocalizacaoRadares();

-- RadaresTipoEnquadramento TRIGGER
CREATE OR REPLACE FUNCTION atualiza_RadaresTipoEnquadramento() RETURNS trigger AS $atualiza_RadaresTipoEnquadramento$
    DECLARE
		m   varchar;
		arr varchar[] := array['R', 'F', 'A', 'P', 'VER', 'V', 'Z', 'PR', 'C', 'EX', 'VR', 'MOTO', 'CP'];
		omgjson json;
    BEGIN
        DELETE FROM camadavisualizacao."RadaresTipoEnquadramento";
		...
		RETURN NEW;
    END;
$atualiza_RadaresTipoEnquadramento$ LANGUAGE plpgsql;

CREATE TRIGGER t_RadaresTipoEnquadramento AFTER INSERT OR UPDATE
ON public."BaseRadares"
FOR EACH STATEMENT EXECUTE PROCEDURE atualiza_RadaresTipoEnquadramento();

-- RadaresZonaConcessao TRIGGER
CREATE OR REPLACE FUNCTION atualiza_RadaresZonaConcessao() RETURNS trigger AS $atualiza_RadaresZonaConcessao$
    DECLARE
	   i integer;
	   arr integer[] := array[1,2,3,4];
	   omgjson json;
    BEGIN
        DELETE FROM camadavisualizacao."RadaresZonaConcessao";
		...
		RETURN NEW;
    END;
$atualiza_RadaresZonaConcessao$ LANGUAGE plpgsql;

CREATE TRIGGER t_RadaresZonaConcessao AFTER INSERT OR UPDATE
ON public."BaseRadares"
FOR EACH STATEMENT EXECUTE PROCEDURE atualiza_RadaresZonaConcessao();

-- DistanciaViagem TRIGGER
CREATE OR REPLACE FUNCTION atualiza_DistanciaViagem() RETURNS trigger AS $atualiza_DistanciaViagem$
    DECLARE
		l1 integer;
		l2 integer;
		j json;
		...
		arr integer[];
	BEGIN
		DELETE FROM camadavisualizacao."DistanciaViagem";
...
-- 		RAISE NOTICE 'DistanciaViagem Processado';
		RETURN NEW;
    END;
$atualiza_DistanciaViagem$ LANGUAGE plpgsql;

CREATE TRIGGER t_DistanciaViagem AFTER INSERT OR UPDATE
ON public.base_radares_lat_lon
FOR EACH STATEMENT EXECUTE PROCEDURE atualiza_DistanciaViagem();
